 ...
 * 项目类型：微信小程序
 * 项目名称：吃对了嘛
 * 项目抓包：抓mc.kukahome.com下的token填入变量
 * 项目变量：lekebo_cdlm_Cookie
 * 项目定时：每40分钟运行一次
 * cron: 0 40 0 * * *
...
let update_data = "完成签到 ,转发 , 评论 ,看视频 任务";
//=======================================//
const $ = new Env("吃对了嘛");
...
let UserCookie = ($.isNode() ? process.env.lekebo_cdlm_Cookie : $.getdata('lekebo_cdlm_Cookie')) || '';
let UserCookieArr = [];
let data = '';
let msg =``;
let ck, ck_status;
    ...
    return new Promise((resolve) => {
        ...
        $.post(url, async (error, response, data) => {
            try {
                ...
                if (task == 1) {
                    DoubleLog(`\n 用户信息: ✅ ，会员：${result.data.baseData.nick}，当前拥有 ${result.data.baseData.score / 100} 元`)
				} else if (task == 2) {
					DoubleLog(`\n 积分查询: ✅ ，${result.data.baseData.score} 积分，约等于 ${result.data.baseData.score / 100} 元`);
                } else {
                    DoubleLog(`\n 登录失败: ❌ ，原因是：${data}`)
                }
            } catch (e) {
        ...
        $.get(url, async (error, response, data) => {
            try {
                ...
                if (result.code == 0) {
		            ...
		            DoubleLog(`\n 任务列表: ❌ ，失败原因是：${result.msg}`)
	            }
				if (task_Arr.share.todayCount < 2) {
			        DoubleLog(`\n 分享转发: ✅ ，转发：${task_Arr.share.todayCount}/2`);
			        await video_list(2 * 1000);
			        await share(2 * 1000);
			        await video_list(2 * 1000);
			        await share(2 * 1000);
		        } else {
				...
				if (task_Arr.comment.todayCount < 2) {
			        DoubleLog(`\n 发表评论: ✅ ，评论：${task_Arr.comment.todayCount}/2`);
			        await video_list(2 * 1000);
			        await add_comment(2 * 1000);
			        await video_list(2 * 1000);
			        await add_comment(2 * 1000);
		        } else {
					DoubleLog(`\n 发表评论: ❌ ，今天已经发表评论完成了`);
				...
				if (task_Arr.video.todayCount < 2) {
					DoubleLog(`\n 广告视频: ✅ ，看视频:${task_Arr.video.todayCount}/2`);
					await video_list(2 * 1000);
					await watch_video(2 * 1000);
					await video_list(2 * 1000);
					await watch_video(2 * 1000);
				} else {
					DoubleLog(`\n 广告视频: ❌ ，今天已经看视频完成了`);
 ...
 * @returns {Promise<unknown>}
 */
function video_list(timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
 ...
 * @returns {Promise<unknown>}
 */
function add_comment(timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
 ...
 * @returns {Promise<unknown>}
 */
function watch_video(timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
    ...
    if ($request.url.indexOf("club-server/front/member/home") > -1) {
		...
		if (ckStr) {
			if (ckStr.indexOf(ck_id) == -1) {  // 找不到返回 -1
				ckStr = ckStr + "@" + ck;
				$.setdata(ckStr, "lekebo_cdlm_Cookie");
				ckList = ckStr.split("@");
				$.msg($.name + ` 获取第${ckList.length}个 ck 成功: ${ck}`);
			}
		} else {
			$.setdata(ck, "lekebo_cdlm_Cookie");
...
async function Envs() {
    if (UserCookie) {
        ...
        console.log(`\n 乐客播提示：系统变量未填写 lekebo_cdlm_Cookie`)
        return;
    }
    return true;
}
