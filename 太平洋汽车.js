...
let update_data = "完成签到，七天签到 3 元红包 , 14 天 2 元红包";
//=======================================//
const $ = new Env('太平洋汽车');
...
let UserCookie = ($.isNode() ? process.env.lekebo_tpyqc_Cookie : $.getdata('lekebo_tpyqc_Cookie')) || '';
let UserCookieArr = [];
let data = '';
let msg =``;
let isSign = false;
...
function getMemberInfo(task,timeout = 2000) {
    return new Promise((resolve) => {
        let url = {
            url: `${hostname}/xsp/s/auto/info/nocache/task/getLoginUserInfo.xsp`,
            headers: {
                'Host': host,
...
async function Envs() {
    if (UserCookie) {
        ...
        console.log(`\n 乐客播提示：系统变量未填写 lekebo_tpyqc_Cookie`)
        return;
    }
    return true;
}
